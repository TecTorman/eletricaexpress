...
public class pedidos extends JFrame {
	...
	public pedidos() {
		...
		setTitle("CER - Controle de pedidos");
		try {
		...
		btnPesquisaData.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (de_data.getDate() != (null)) {
					...
					de_data_ = sdf.format(de_data.getDate());
					ate_data_ = sdf.format(ate_data.getDate());
				}
	...
	public static void carregar_pedidos() {
		...
		try {
			...
			String sSQL = "SELECT p.Codigo as N,p.Status, a.Nm_Cliente as Solicitante,a.Tipo,p.Dt_Atend as Agendado,a.VL_Obra as Instalacao,a.VL_Prod as Produtos,a.VL_Total as Total FROM agenda a INNER JOIN pedido p ON a.Codigo=p.Codigo_Orca Order by p.Codigo desc;";
	...
	public static void pesquisa_por_data(String de_data_, String ate_data_) {
		...
		try {
			Connection ExConn = (Connection) connect.abrirBDConn();
			Statement stmt = (Statement) ExConn.createStatement();
			String sSQL = "SELECT p.Codigo as N,p.Status, a.Nm_Cliente as Solicitante,a.Tipo,p.Dt_Atend as Agendado,a.VL_Obra as Instalacao,a.VL_Prod as Produtos,a.VL_Total as Total FROM agenda a INNER JOIN pedido p ON a.Codigo=p.Codigo_Orca  WHERE p.Dt_Atend >='"
					+ de_data_ + "' and p.Dt_Atend <='" + ate_data_ + "' order by p.Codigo desc;";
			System.out.println(sSQL);
	...
	public static void pesquisa_por_status(String status) {
		...
		try {
			Connection ExConn = (Connection) connect.abrirBDConn();
			Statement stmt = (Statement) ExConn.createStatement();
			String sSQL = "SELECT p.Codigo as N,p.Status, a.Nm_Cliente as Solicitante,a.Tipo,p.Dt_Atend as Agendado,a.VL_Obra as Instalacao,a.VL_Prod as Produtos,a.VL_Total as Total FROM agenda a INNER JOIN pedido p ON a.Codigo=p.Codigo_Orca  WHERE p.Status='"
